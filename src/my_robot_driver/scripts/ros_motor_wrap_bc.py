...
from my_robot_driver.msg import Motor_Info
from geometry_msgs.msg import Twist

class MotorDriverROSWrapper:
    def __init__(self):
        max_speed = rospy.get_param("~max_speed", 75)
        publish_motor_info_frequency = rospy.get_param("~publish_motor_info_frequency", 5.0)
    ...
    def publish_motor_status(self, event=None):
        state, speed = self.motor.move_command('fw', 0)
        msg = Motor_Info()
        msg.motor_state = state
        msg.speed = speed
    ...
    def callback_speed_command(self, data):
        self.motor.move_command(data.motor_state, data.speed)
